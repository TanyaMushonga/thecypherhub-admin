...
function ArticleList() {
  ...
  const { loading, articles, setArticles, setPage, totalCount, hasMore } =
    useFetchArticles("all", debouncedSearch);
    ...
    <div className="space-y-6 w-full max-w-4xl">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between md:items-center bg-blue-950 p-4 rounded-lg border border-blue-900">
        ...
        <div className="relative w-full md:w-1/3">
          ...
          {loading && (
            <div className="absolute right-2 top-2.5 h-4 w-4 rounded-full border-2 border-slate-400 border-t-transparent animate-spin"></div>
          )}
          <Input
      ...
      <div className="space-y-4">
                ...
                <div className="flex items-center gap-1">
                  <Sheet>
                    <SheetTrigger asChild>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 gap-2 text-slate-400 hover:text-white hover:bg-blue-800"
                      >
                      ...
                      <div className="space-y-4 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
                        {article.comments?.length > 0 ? (
                          article.comments.map((comment) => (
                            <div
                              key={comment.id}
                              className="p-3 bg-blue-900/30 rounded border border-blue-900/50"
                            >
                  ...
                  <Link href="/article/[slug]" as={`/article/${article.slug}`}>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-8 w-8 text-slate-400 hover:text-white hover:bg-blue-800"
                    >
        ...
        <div
          ref={observerTarget}
          className="h-4 w-full flex justify-center p-4"
        >
          {loading && hasMore && (
            <span className="animate-pulse text-slate-500 text-xs">
              Loading more...
            </span>
          )}
        </div>
